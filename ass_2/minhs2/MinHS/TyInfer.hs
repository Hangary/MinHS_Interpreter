...
inferProgram env [Bind "main" userType [] body] = do
  (body', t, s) <- inferExp env body
  let t' = s `substitute` t
      body'' = allTypes (substQType s) body'
      t'' = generalise env t'
      main = Bind "main" (Just t'') [] body''
  return ([main], t', s)
  
-- error "implement me! don't forget to run the result substitution on the"
